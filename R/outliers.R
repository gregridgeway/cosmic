...
.officer_summary_from_lambda_draws <- function(object,
  ...
  rows <- lapply(seq_len(n_off), function(i_id) {
    i_peers <- setdiff(which(Sigma_diff[i_id, ] < var_threshold), i_id)
    n_peers <- length(i_peers)

    force_counts <- stats::setNames(
      as.list(tabulate(object$data$y[object$data$idOff == i_id],
                       nbins = n_force_types)),
      force_names
    )
    ...
    if (n_peers == 0L) {
      ...
      ranks <- apply(lambda_draws[, c(i_id, i_peers), drop = FALSE], 1, rank1)
      lam_contrast <- lambda_draws[, i_id] -
        rowMeans(lambda_draws[, i_peers, drop = FALSE])

      stats <- list(
        pRankToppct = mean(ranks >= (1 - pct_tail) * (n_peers + 1)),
        pRankBotpct = mean(ranks <= pct_tail * (n_peers + 1)),
    ...
    return(as.data.frame(c(list(idOff = i_id,
                                nPeers = n_peers,
                                nInc = sum(object$data$idOff == i_id)),
                           force_counts,
...
#'
#' @param x A \code{"cosmic_officer_summary"} object, data frame returned by
#'   \code{\link{officer_summary}}, or a fitted \code{"cosmic_fit"} object.
#' @param prob_outlier Probability cutoff used to flag an outlier. Default is
#'   \code{0.8}.
#' @param ... Additional arguments passed to \code{\link{officer_summary}} when
#'   \code{x} is a fitted \code{"cosmic_fit"} object.
#'
...
outlier_report <- function(x, prob_outlier = 0.8, ...) {
  ...
  if (!is.data.frame(x)) {
    stop("x must be a data frame, cosmic_officer_summary, or cosmic_fit object",
         call. = FALSE)
  }

  if (!is.numeric(prob_outlier) || length(prob_outlier) != 1L ||
      is.na(prob_outlier) || prob_outlier <= 0 || prob_outlier >= 1) {
    stop("prob_outlier must be a numeric scalar between 0 and 1",
         call. = FALSE)
  }
